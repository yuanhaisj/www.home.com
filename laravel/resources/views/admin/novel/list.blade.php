                    ...
                    @if(!empty($novels))
                      @foreach($novels as $novel)
                        <tr>
                          ...
                          <td><img src="{{$novel['image_url']}}" alt="" style="width: 50px;height: 50px;"></td>
                          <td>{{$novel['name']}}</td>
                          <td>{{$novel['status']==1 ? "连载" : "完结"}}</td>
                          <td>
                              <a href="{{route('admin.novel.edit',['id'=>$novel['id']])}}" class="btn btn-sm btn-primary">编辑</a>&nbsp;
                              <a href="{{route('admin.chapter.create',['id'=>$novel['id']])}}" class="btn btn-sm btn-primary">章节添加</a>&nbsp;
                              <a href="{{route('admin.chapter.list',['id'=>$novel['id']])}}" class="btn btn-sm btn-primary">章节查看</a>&nbsp;
